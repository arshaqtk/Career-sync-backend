...
export const CandidategetJobs = async (req: Request, res: Response) => {
  ...
  const { page = 1, limit = 10, location, jobType, status, search, remote, experienceMin, experienceMax, field, recommended } = req.query
  // if(!id){ 
  //      throw new CustomError("unAuthorized User Not Found",401)
  // }
  const result = await CandidategetJobsService({
    candidateId: id, query: {
      page: Number(page),
      limit: Number(limit),
